                    ...
                    <tbody>
                        @foreach($invitations as $inv)
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="badge {{ $inv->invited ? 'badge-success' : 'badge-warning' }}">
                                        {{ $inv->invited ? 'Sí' : 'No' }}
                                    </span>
                                </td>
                                <td>
                                    <span class="badge {{ $inv->confirmed_at ? ($inv->confirmed ? 'badge-success' : 'badge-danger') : 'badge-warning' }}">
                                        {{ $inv->confirmed_at ? ($inv->confirmed ? 'Confirmado' : 'No asistirá') : 'Pendiente' }}
                                    </span>
                                </td>
                                ...
                                <td>
                                    <div class="relative flex justify-end" x-data="{ menuOpen: false }" @click.away="menuOpen = false" @keydown.escape="menuOpen = false">
                                        <button type="button" @click="menuOpen = !menuOpen"
                                                class="btn btn-ghost btn-icon btn-sm" title="Acciones">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <div x-show="menuOpen" x-cloak x-transition.origin.top.right
                                             class="absolute right-0 top-full mt-1 w-56 bg-white rounded-xl border border-surface-100 shadow-lg py-1.5 z-20 text-left">

                                            {{-- WhatsApp --}}
                                            <button type="button"
                                                    @click="menuOpen = false; openWa({{ json_encode($inv->guest_name) }}, {{ json_encode($waMsg) }})"
                                                    class="w-full flex items-center gap-2.5 px-3.5 py-2 text-sm text-surface-700 hover:bg-surface-50">
                                                <svg class="w-4 h-4 text-[#25D366] flex-shrink-0" viewBox="0 0 24 24" fill="currentColor">
                                            ...
                                            <a href="{{ $inv->invitation_url }}" target="_blank"
                                               class="flex items-center gap-2.5 px-3.5 py-2 text-sm text-surface-700 hover:bg-surface-50">
                                            ...
                                            {{-- Confirmación manual (solo admin/super-admin) --}}
                                            @role('super-admin|admin')
                                                @unless($inv->confirmed_at)
                                                    <div class="my-1.5 border-t border-surface-100"></div>

                                                    <form method="POST" action="{{ route('admin.invitations.confirm', $inv) }}"
                                                          x-data @submit.prevent="if(confirm('¿Marcar {{ $inv->guest_name }} como asistirá?')) $el.submit()">
                                                        @csrf
                                                        <input type="hidden" name="confirmed" value="1">
                                                        <input type="hidden" name="confirmed_guests" value="{{ $inv->allowed_guests }}">
                                                        <button type="submit" class="w-full flex items-center gap-2.5 px-3.5 py-2 text-sm text-success-700 hover:bg-success-50">
                                                            <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/>
                                                            </svg>
                                                            Confirmar asistencia (admin)
                                                        </button>
                                                    </form>
                                                    <form method="POST" action="{{ route('admin.invitations.confirm', $inv) }}"
                                                          x-data @submit.prevent="if(confirm('¿Marcar {{ $inv->guest_name }} como no asistirá?')) $el.submit()">
                                                        @csrf
                                                        <input type="hidden" name="confirmed" value="0">
                                                        <button type="submit" class="w-full flex items-center gap-2.5 px-3.5 py-2 text-sm text-danger-600 hover:bg-danger-50">
                                                            <svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/>
                                                            </svg>
                                                            Marcar no asistirá (admin)
                                                        </button>
                                                    </form>
                                                @endunless
                                            @endrole
