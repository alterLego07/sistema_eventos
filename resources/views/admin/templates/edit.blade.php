    ...
    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-2xl border border-surface-100 shadow-sm overflow-hidden">
            <div class="px-6 py-5 border-b border-surface-100">
                ...
                <p class="text-sm text-surface-400 mt-0.5">Modificá la configuración de la plantilla.</p>
            </div>

            <form method="POST" action="{{ route('admin.templates.update', $template) }}" class="px-6 py-6 space-y-6">
                @csrf @method('PATCH')

                <div class="space-y-4">
                    <div>
                        <label class="form-label" for="name">Nombre <span class="text-danger-500">*</span></label>
                        <input id="name" type="text" name="name" value="{{ old('name', $template->name) }}"
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="2"
                                  class="form-input">{{ old('description', $template->description) }}</textarea>
                    </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                        @foreach([
                            ...
                            @php $value = old("configuration.colors.$key", $template->configuration['colors'][$key] ?? '#000000'); @endphp
                            <div>
                                <label class="form-label text-xs">{{ $label }}</label>
                                <div class="flex items-center gap-2">
                                    <input type="color" name="configuration[colors][{{ $key }}]"
                                           value="{{ $value }}"
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" name="configuration[fonts][heading]"
                                   value="{{ old('configuration.fonts.heading', $template->configuration['fonts']['heading'] ?? '') }}"
                                   class="form-input">
                        </div>
                        <div>
                            <label class="form-label">Cuerpo</label>
                            <input type="text" name="configuration[fonts][body]"
                                   value="{{ old('configuration.fonts.body', $template->configuration['fonts']['body'] ?? '') }}"
                                   class="form-input">
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        @foreach(['hero', 'message', 'details', 'countdown', 'rsvp', 'location', 'song', 'dietary', 'footer'] as $section)
                            @php $checked = in_array($section, $template->configuration['sections'] ?? []); @endphp
