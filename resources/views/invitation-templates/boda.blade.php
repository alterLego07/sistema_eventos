@php
    ...
    $heroText = $config['texts']['hero']
        ?? 'Queremos compartir con vos un día muy especial, lleno de amor, alegría y momentos que vamos a recordar para siempre.';

    $messageTitle = $config['texts']['message_title'] ?? 'Con mucho amor';
    $messageText = $event->description
        ?? $config['texts']['message']
        ?? 'Después de tantos momentos compartidos, llegó el día de celebrar nuestra unión junto a las personas que forman parte de nuestra historia.';
    ...
    $details = $config['details'] ?? [
        ...
        [
            ...
            'url' => $config['reception']['location_url'] ?? null,
            'button' => 'Ver ubicación',
        ],
    ];
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        .nav {
            ...
            border-radius: 999px;
            background: rgba(178, 207, 157, .72);
            backdrop-filter: blur(18px);
            box-shadow: 0 14px 35px rgba(47, 68, 37, .12);
        }

        .brand { display: inline-flex; align-items: center; gap: 10px; font-weight: 700; white-space: nowrap; }
        .brand__icon {
            width: 34px; height: 34px; display: grid; place-items: center;
            border-radius: 50%; color: var(--gold); background: rgba(255,255,255,.42); border: 1px solid var(--line);
        }

        .nav__links { display: flex; align-items: center; gap: 18px; font-size: 14px; color: var(--green-muted); }
        .nav__links a:hover { color: var(--green-deep); }
        .nav__cta {
            padding: 10px 14px; border-radius: 999px; color: var(--green-deep) !important;
            background: var(--paper-strong); border: 1px solid rgba(122, 92, 18, .26);
        }
        ...
        .nav-toggle span { display: block; width: 18px; height: 2px; margin: 5px auto; border-radius: 999px; background: var(--green-deep); }

        .section { padding: 72px 0; }
        .section-pad { padding: 86px 0 56px; }
        .hero { min-height: calc(100svh - 82px); display: grid; align-items: center; }
        .hero__grid { display: grid; grid-template-columns: minmax(0, 1.1fr) minmax(280px, .72fr); gap: 34px; align-items: center; }
        ...
        .hero__text { max-width: 640px; margin: 26px 0 0; color: var(--green-muted); font-size: clamp(16px, 2vw, 19px); }
        ...
        .date-card {
            ...
            align-content: center; padding: 36px 26px; text-align: center;
        }
        ...
        .date-card__month, .date-card__year { font-family: var(--title); font-size: 28px; }
        .date-card__day { margin: 8px 0; font-family: var(--title); font-size: 112px; line-height: .9; color: var(--gold); }
        ...
        .details-grid, .rsvp-grid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 24px; }
        .rsvp-grid { grid-template-columns: .72fr 1.28fr; align-items: start; }
        ...
        .greeting-section { padding: 72px 0 0; }
        .greeting-card {
            border: 1px solid var(--line); border-radius: var(--radius); background: var(--paper);
            box-shadow: var(--shadow); backdrop-filter: blur(14px);
        ...
        .greeting-name {
            font-family: var(--script); font-size: clamp(56px, 10vw, 108px);
            line-height: .9; color: var(--green-deep); margin: 0 0 16px;
        }
        .greeting-text {
            color: var(--green-muted); font-size: clamp(15px, 2vw, 18px); margin: 0; max-width: 560px; margin-inline: auto;
        }
        ...
        @media (max-width: 900px) {
            ...
            .nav__links { display: none; width: 100%; padding: 8px 0 2px; flex-direction: column; align-items: stretch; gap: 8px; }
            .nav.is-open .nav__links { display: flex; }
            .nav__links a { padding: 12px 14px; border-radius: 16px; background: rgba(255,255,255,.26); }
            .nav__cta { text-align: center; }
            .hero__grid, .details-grid, .rsvp-grid { grid-template-columns: 1fr; }
            .hero { min-height: auto; }
            .section-pad { padding-top: 62px; }
            .date-card { min-height: 330px; }
        }
        ...
        @media (max-width: 560px) {
            ...
            .section-pad { padding: 54px 0 42px; }
            .couple-name { font-size: clamp(64px, 22vw, 94px); }
            .hero__actions .btn { width: 100%; }
            .date-card__day { font-size: 86px; }
...
<header class="site-header" id="inicio">
    <nav class="nav container" aria-label="Navegación principal">
        <a class="brand" href="#inicio" aria-label="Volver al inicio">
            ...
            <span>{{ $config['brand'] ?? 'Nuestra Boda' }}</span>
        </a>
        ...
        <div class="nav__links" id="navLinks">
            @if(in_array('message', $sections)) <a href="#historia">Historia</a> @endif
            @if(in_array('details', $sections)) <a href="#detalles">Detalles</a> @endif
            @if(in_array('countdown', $sections)) <a href="#cuenta-regresiva">Cuenta regresiva</a> @endif
            @if(in_array('rsvp', $sections)) <a href="#confirmacion" class="nav__cta">Confirmar</a> @endif
        </div>
    </nav>
</header>
...
<main>
    ...
    <section class="greeting-section container">
        <div class="greeting-card reveal">
            <p class="greeting-salute">Con mucho cariño</p>
            <p class="greeting-name">{{ $invitation->guest_name }}</p>
            <p class="greeting-text">{{ $heroText }}</p>
        </div>
    </section>
                ...
                <div class="hero__content reveal">
                    ...
                    <div class="hero__actions">
                        @if(in_array('rsvp', $sections))
                            <a class="btn" href="#confirmacion">Confirmar asistencia</a>
                        @endif
                        @if(in_array('details', $sections))
                            <a class="btn btn--soft" href="#detalles">Ver detalles</a>
                        @endif
                    </div>
                </div>
            ...
            <div class="details-grid">
                @foreach($details as $detail)
                    <article class="detail-card reveal">
                        ...
                        @if(!empty($detail['url']))
                            <a class="btn btn--full btn--soft" href="{{ $detail['url'] }}" target="_blank" rel="noopener">
            ...
            <div class="rsvp-grid">
                <article class="detail-card reveal">
                    ...
                    <ul class="clean-list">
                        ...
                        @endif
                        <li>Esta invitación es válida para <strong>{{ $invitation->allowed_guests ?? 1 }}</strong> {{ ($invitation->allowed_guests ?? 1) == 1 ? 'persona' : 'personas' }}.</li>
                    </ul>
                </article>
